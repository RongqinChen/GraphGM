...
@torch.no_grad()
def compute_mixed_middle_bernstein_polynomials(
    ...
    adj2 = adj2.to_dense() / 2.0

    identity = torch.eye(num_nodes)
    basis = [adj1 @ adj2]

    while len(basis) < power:
        base = basis[-1]
        base = base @ base
        basis.append(base)

    basis = [base * comb((k + 1) * 2, k + 1) for k, base in enumerate(basis)]
