...
def add_full_gm1(
    ...
    adj = adj.to_dense()
    poly_list = [torch.eye(num_nodes, dtype=torch.float), adj] + [None] * (poly_order - 2)
    for k in range(2, poly_order):
        ldx, rdx = (k) // 2, (k + 1) // 2
        poly_list[k] = poly_list[ldx] @ poly_list[rdx]

    gm_list = poly_list + [
        poly_list[idx] * poly_list[jdx]
        for idx in range(1, poly_order)
        for jdx in range(idx, poly_order)
    ]
