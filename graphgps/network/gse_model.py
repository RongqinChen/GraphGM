...
class FeatureEncoder(torch.nn.Module):
    ...
    def __init__(self, dim_in):
        ...
        if cfg.dataset.node_encoder:
            ...
            self.dim_in = cfg.gse_model.hidden_dim
        if cfg.dataset.edge_encoder:
            # Hard-limit max edge dim for PNA.
            if "PNA" in cfg.gt.layer_type:
                cfg.gse_model.dim_edge = min(128, cfg.gse_model.hidden_dim)
            else:
                cfg.gse_model.dim_edge = cfg.gse_model.hidden_dim
            # Encode integer edge features via nn.Embeddings
            EdgeEncoder = register.edge_encoder_dict[cfg.dataset.edge_encoder_name]
            self.edge_encoder = EdgeEncoder(cfg.gse_model.dim_edge)
...
class GseModel(torch.nn.Module):
    ...
    def __init__(self, dim_in, dim_out):
        super().__init__()
        assert cfg.posenc_Poly.emb_dim >= 2 ** (cfg.gse_model.messaging.num_layers)
        self.encoder = FeatureEncoder(cfg.gse_model.hidden_dim)
        self.init_layer = InitialLayer(cfg.gse_model.hidden_dim)

        GseMessagingBlock = register.layer_dict["GseMessagingBlock"]
        GseFullBlock = register.layer_dict["GseFullBlock"]
        AbsEncoder = register.node_encoder_dict["poly"]
        RelEncoder = register.edge_encoder_dict["poly"]

        self.block_dict = nn.ModuleDict()
        for lidx in range(1, cfg.gse_model.messaging.num_layers + 1):
            emb_dim = 2**lidx
            abs_encoder = AbsEncoder(cfg.posenc_Poly.method, emb_dim, cfg.gse_model.hidden_dim)
            rel_encoder = RelEncoder(cfg.posenc_Poly.method, emb_dim, cfg.gse_model.hidden_dim, pad_to_full=False,)
        ...
        if cfg.gse_model.full.num_layers > 0:
            abs_encoder = AbsEncoder(cfg.posenc_Poly.method, cfg.posenc_Poly.emb_dim, cfg.gse_model.hidden_dim, )
            rel_encoder = RelEncoder(cfg.posenc_Poly.method, cfg.posenc_Poly.emb_dim, cfg.gse_model.hidden_dim, pad_to_full=True, )
        ...
        if cfg.posenc_Poly.method == "mixed_bern":
            ...
            self._poly_order_map = {
                lidx: 2**(lidx - 1)
                for lidx in range(2, cfg.gse_model.messaging.num_layers + 1)
            }
        else:
    ...
    def forward(self, batch: Batch):
        ...
        whole_poly_index = batch[f"{cfg.posenc_Poly.method}_index"]
        ...
        for lidx in range(1, cfg.gse_model.messaging.num_layers + 1):
            if lidx == 1:
                abs_h = self.block_dict[f"{lidx}_abs_enc"](whole_abs_val[:, :2])
                batch.x = batch.x + abs_h
                order1_index = whole_poly_index[:, order1_flag]
                order1_val = whole_poly_val[order1_flag, :2]
                # order1_val = whole_poly_val[:, :2][order1_flag, :]
                order1_h = self.block_dict[f"{lidx}_rel_enc"](order1_val)
                poly_idx, poly_val = torch_sparse.coalesce(
                    torch.cat([batch.edge_index, order1_index], dim=1),
                    torch.cat([batch.edge_attr, order1_h], dim=0),
                    batch.num_nodes, batch.num_nodes, op="add",
                )
                batch.poly_index = poly_idx
                batch.poly_val = poly_val
            else:
                order = self._poly_order_map[lidx]
                abs_h = self.block_dict[f"{lidx}_abs_enc"](whole_abs_val[:, :order])
                batch.x = batch.x + abs_h
                flag = whole_poly_val[:, order] > 0
                poly_idx = whole_poly_index[:, flag]
                # part_poly_val = whole_poly_val[:, :order + 1]
                poly_val = whole_poly_val[flag, :order + 1]
                poly_h = self.block_dict[f"{lidx}_rel_enc"](poly_val)
                poly_idx, poly_val = torch_sparse.coalesce(
                    torch.cat([batch.poly_index, poly_idx], dim=1),
        ...
        if cfg.gse_model.full.num_layers > 0:
            ...
            poly_idx, poly_val = torch_sparse.coalesce(
                torch.cat([batch.poly_index, full_edge_index], dim=1),
